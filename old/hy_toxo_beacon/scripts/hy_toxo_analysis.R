...
###############################################################################
  ...
  ### 3.3 Tidy life_hist
    ...
    #              remove = T) %>%
    #     separate(col = disappeared, into = c('disappeared.date', 
    ...
    #                                     format = '%Y-%m-%d')
    #   
    #   life_hist$disappeared.date <- as.Date(life_hist$disappeared.date, 
    #                                       format = '%Y-%m-%d')
    #   
...
#***************************  Toxo Hyena Survival  *****************************      
  ...
  ### 4.9 Subset /tidy luma_data_lf_hist for mortality analyses
    ...
    ## c) Subset the toxo data to include only hyenas which are known dead
      toxo_mort <- toxo_data  %>% 
        filter(!is.na(disappeared.event.data) & !grepl('unk*|^double|infe*',
                                                    disappeared.event.data))
...
#$$$$$$$$$$$$$$$$$$$$$$$ NO SURVIVAL DATA PREPARED $$$$$$$$$$$$$$$$$$$$$$$$$$$$      
  ...
  #       filter(!grepl('unk*|^double', disappeared.event.data)) %>%
