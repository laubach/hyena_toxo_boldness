...
### Hyena project.
  ...
  # Subsections corresponding to the hypotheses being tested begin with the 
  # the following line break labels in code blocks 4-7   
...
###############################################################################
  ...
  ### 1.3 Install and load CRAN packages
    ...
    ## b) Graph Plotting and Visualization Packages
      ...
      # load gridExtra package (used with grid.table)
        library ('gridExtra')
...
###############################################################################    
  ...
  ### 2.1 Import toxo data files
    ...
    ## b) Import full lion distances data
...
###############################################################################
  ...
  ### 3.3 Tidy life_hist
    ...
    ## h) HACK: remove duplicated entries from life_hist
      # Includes:
      # 14769	nmlk	disappeared	2007-12-04	dls	NA, also has event.code 'dead'
      # 14768	u859	disappeared	2005-05-27	dls, also has event.code 'dead'
      # in both cases retain row where event.code is 'dead' and not 'dls'
      life_hist <- life_hist %>%
        filter(!grepl('nmlk', hy.id) | !grepl('dls', event.status)) %>%
        filter(!grepl('u859', hy.id) | !grepl('dls', event.status))         
      
    ## i) Transponse life_hist from long to wide
      life_hist <- life_hist %>%
        unite(col = info, date, event.status, event.data, sep = ';') %>%
        #distinct(hy.id, event.code, .keep_all = T) %>%
        spread(key = event.code, value = info)
      
    ## j) Separate united info into spearate variables/columns
      life_hist <- life_hist %>%
    ...
    ## k) Convert dates stored as character (e.g. 8/3/05) to yyyy-mm-dd format
  ...
  ### 3.8 Tidy data with precise distance
    ...
    ## c) Tally total number of hyena lion sessions in raw data
      lion_hy_dist_sum <- lion_hy_dist_raw %>%
      group_by(session) %>%
        summarize (n.session = sum(!is.na(session)))
...
#        filter(grepl('[^[:alnum:]]', distance))
  ...
  ### 3.10 Tidy data lion_hy_dist     
    ## a) Rbind the two filtered data frames in which distances were 
      # originally recorded as ranges/inequalites to the data frame containing 
...
###############################################################################      
  ...
  # Subsections corresponding to the hypotheses being tested begin with the 
  # the following line break labels    
    #************************  Determinants of Toxo  *************************
    #***********************  Toxo Lion Hyena Dist.  *************************
    #************************  Toxo Hyena Survival  **************************      
...
#***************************  Determinants of Toxo  **************************** 
  ...
  ## ***********DATA CLEANING*************
    ## i) Clean data      
      ...
      # IFAT correlation
      ## a) Subset questionalbe data (ELISA and IFAT discordance)
      toxo_data_gil_baj <- toxo_data %>%
        filter(kay.code == '87' | kay.code == '84')
      
      ## b) Remove questionalbe data (ELISA and IFAT discordance)
      toxo_data <- toxo_data %>%
        filter(kay.code != 87 & kay.code != 84)
...
#**************************  Toxo Lion Hyena Dist.  ****************************  
  ...
  ### 4.4 Join hyenas and life_hist ables to lion_hy_dist
  ...
  ### 4.5 Tidy lion_hy_dist and prepare data for analysis   
    ...
    ## f) Make a new human disturbance variable
      # create a 2-level ordinal factor indicating human pastoralist presence /
      # disturbance based Green et. al 2017
      # Note gap between 2000 and 2012 limits matches sampling design 
      # selecting hyena plasma samples for diagnosis based partially on hi vs
  ...
  ### 4.6 Join lion_hy_dist with toxo_data      
    ## a) Join select varaibles from toxo_data to lion_hy_dist as new dataframe
      lion_hy_dist_toxo <- lion_hy_dist %>%
        left_join(select(toxo_data, c(hy.id, hum.pop.den, diagnosis, dart.date, 
                                      age.mon.dart, age.cat.dart)), 
                  by = c('hy.id' = 'hy.id'))
    ...
    ## e) Avoid misdiagnosed behaviors
      ...
      #*** NOTE *** This is a data inclusion cut-off decision for sensitivity
      # analyses
      
      
  ## 4.7 Age stratification 
      ## ****** NOTE: After determining significant age structure in hyena 
      # T. gondii infection prevalence and approach distances towards lions,  
      # we stratified the data based on age
...
#***************************  Determinants of Toxo  ****************************
  ...
  ### 5.1 Univariate stats toxo 
    ...
    ## b) save the data frame of summary stats out as a pdf into output file
      pdf(here::here('output/univar_toxo_status.pdf'), height = 4, width = 8)
    ...
    ## f) Save Plot
       ...
       ggsave('spratio_histogram.pdf', plot = last_plot(), device = NULL,
              path = here::here('output/'), scale = 1, width = 5,
              height = 3,
...
#**************************  Toxo Lion Hyena Dist.  ****************************  
  ...
  ### 5.3 Univariate stats avg minimum approach towards lions 
      ...
      univar_min_app_dist_sum <- lion_hy_dist_toxo_sum %>%
        summarize(n.app.dist = sum(!is.na(avg.min.dist.lion)),
                  ...
                  sd.app.dist = round(sd(avg.min.dist.lion, 
                                         na.rm = T), 2))
    ## b) save the data frame of summary stats out as a pdf into output file
        pdf(here::here('output/univar_min_app_dist_sum.pdf'), height = 3, width = 8)
        grid.table(univar_min_app_dist_sum)
        dev.off()
    ...
    ## f) Histogram Outcome (avg minimum approach distance)
...
#***************************  Determinants of Toxo  ****************************  
  ...
  ### 6.1 Descriptive bivariate stats toxo status by sex
    ...
    ## b) save the data frame of summary stats out as a pdf into output file
      pdf(here::here('output/sex_ratio_summary.pdf'), 
          height = 3, width = 5)
      grid.table(sex_ratio_summary)
      dev.off() 
      
    # ## c) Bivariate descriptive stats for sex
    #   # Use filter version if there is missing data for grouping variable; 
  ...
  ### 6.2 Descriptive bivariate stats toxo by age.mon.dart    
    ...
    ## b) save the data frame of summary stats out as a pdf into output file
      pdf(here::here('output/age_var_summary.pdf'), 
          height = 4, width = 5)
    ...
    ## d) save the data frame of summary stats out as a pdf into output file
      pdf(here::here('output/age_cat_summary.pdf'), 
          height = 3, width = 5)
    ...
    ## i) Save Plot
      ...
      ggsave('age_dart_by_toxo_plot.pdf', plot = last_plot(), device = NULL, 
             path = here::here('output/'), scale = 1, width = 7, 
             height = 5, 
             units = c('in'), dpi = 300, limitsize = TRUE)  
      
  ### 6.3 Descriptive bivariate stats toxo by human pop size
    ...
    ## b) save the data frame of summary stats out as a pdf into output file
      pdf(here::here('output/hum_pop_summary.pdf'), 
          height = 3, width = 5)
...
##******************* Determinants hyena bevavior w/ lions *******************##  
  ...
  ### 6.5 Bivariate descriptive stats for approach distances
    ...
    ## b) save the data frame of summary stats out as a pdf into output file
      pdf(here::here('output/dist_by_sex_summary.pdf'),
          height = 3, width = 7)
    ...
    ## e) Plot approach distance by sex
...
#   ***************** Boxplot END *****************
    ...
    ## f) Save Plot
      ...
      ggsave('app_dist_by_sex_plot.pdf', plot = last_plot(), device = NULL,
             path = paste0(here::here(),'/output'),
             scale = 1, width = 11, height = 7,
             units = c('in'), dpi = 300, limitsize = TRUE)

   
     ## g) Bivariate descriptive stats for approach distance by age
       # calculate n, mean, median and standard deviation; repeat measures per ID
       bivar_dist_by_age_rep <- lion_hy_dist_toxo %>%
    ...
    ## h) save the data frame of summary stats out as a pdf into output file
      pdf(here::here('output/dist_by_age_summary.pdf'),
          height = 3, width = 7)
      grid.table(bivar_dist_by_age_rep)
      dev.off()

    # ## i) Bivariate descriptive stats for approach distance by age
    #   # when data are restricted (before seronegative, after seropositive)
    ...
    #                sd.app.dist = round(sd(avg.min.dist.lion, na.rm = T), 2))
    # 
    # ## j) Bivariate descriptive stats for approach distance by age.cat.lion
    # # calculate n, mean, median and standard deviation grouped by age.
    ...
    ## k) Plot approach distance by age (categorical)
      # NOTE: These summaries contain non-independent measures of app distances
...
#   ***************** Boxplot END *****************
    ...
    ## m) Plot approach distance by age.mon.lion
      # NOTE: These summaries contain non-independent measures of app distances
...
#   *************** Scatterplot END ***************
    ...
    ## p) save the data frame of summary stats out as a pdf into output file
      pdf(here::here('output/dist_by_fd_pres_summary.pdf'), 
          height = 3, width = 7)
    ...
    # ## u) Bivariate descriptive stats for approach distance by human disturb.
    #   # calculate n, mean, median and standard deviation grouped by age.
    ...
    #   dev.off() 
    #   
    # ## w) Bivariate descriptive stats for approach distance by human disturb.
    #   # when data are restricted (before seronegative, after seropositive)
    ...
    ## v) save the data frame of summary stats out as a pdf into output file
      pdf(here::here('output/dist_by_hum_dist_summary.pdf'), 
          height = 3, width = 7)
...
#   ***************** Boxplot END *****************
  ...
  ### 6.7 Descriptive stats min approach distance towards lions by toxo status
    ...
    ## b) save the data frame of summary stats out as a pdf into output file
      pdf(here::here('output/app_dist_by_toxo_full_summary.pdf'), 
          height = 3, width = 10)
  ...
  ### 6.8 Bivariate descriptive stats for AGE STRATIFIED approach distances
    ...
    ## b) save the data frame of summary stats out as a pdf into output file
      pdf(here::here('output/bivar_cub_dist_by_toxo_sum'), 
          height = 3, width = 7)
    ...
    ## d) save the data frame of summary stats out as a pdf into output file
      pdf(here::here('output/bivar_sub_adult_dist_by_toxo_sum'), 
          height = 3, width = 7)
...
##****** NOTE: After identifiying age structure in hyena approach distance
  ...
  # ### 7.1 Age (at hyena lion interaction) z-score standardization of 
  #     # approach distances
  #     
...
###############################################################################     
  ...
  ##***NOTE: LSmeans calculated in SAS    
  ...
  ### 8.1 Boldness towards lions: Approach lion distance by sex
    ...
    ## b) Min. distance from lions by sex using 'nmle' package, 
      #which will provide p-value estimates
      sex.lme <- lme(distance ~ sex, random =~1|hy.id,
                     subset(lion_hy_dist_toxo,!is.na(x = sex)))
     
      summary(sex.lme)
      intervals(sex.lme, which = 'fixed')

      
    # ## c) Repeated measures minimum approach distance RESTRICTED data
    #   # min dist lions data by sex
    ...
    # ## d) Lifetime average minimum approach distance (summarized data) 
    #   # FULL data set
  ...
  ### 8.2 Boldness towards lions: Approach lion distance by age
    ...
    ## b) Min. distance from lions by age.cat.lion using 'nmle' package, 
      # which will provide p-value estimates
      age.lme <- lme(distance ~ age.cat.lion, random =~1|hy.id,
                     subset(lion_hy_dist_toxo,!is.na(x = age.cat.lion)))
      
      summary(age.lme)
      intervals(age.lme, which = 'fixed')
  ...
  ### 8.3 Boldness towards lions: Approach lion distance by food present
    ...
    ## b) Min. distance from lions by food.present using 'nmle' package, 
      # which will provide p-value estimates
      fd.lme <- lme(distance ~ food.present, random =~1|hy.id,
                     subset(lion_hy_dist_toxo,!is.na(x = food.present)))
      
      summary(fd.lme)
      intervals(fd.lme, which = 'fixed')
  ...
  ### 8.4 Boldness towards lions: Approach lion distance by hum.pop.den
    ## a) # Repeated measures minimum approach distance FULL data
    ...
    ## b) Min. distance from lions by hum.dist.lion using 'nmle' package, 
      # which will provide p-value estimates
      hum.pop.lme <- lme(distance ~ hum.dist.lion, random =~1|hy.id,
                    subset(lion_hy_dist_toxo,!is.na(x = hum.dist.lion)))
      
      summary(hum.pop.lme)
      intervals(hum.pop.lme, which = 'fixed')
...
###############################################################################    
  ...
  ### 9.2 Unadujusted model: Toxo by age 
    # ## a) Logistic regression toxo_status by age
  ...
  ### 9.3 Unadujusted model: Toxo by human disturbance   
    ## a) Logistic Regression toxo_status by hum_pop_den
      hum.pop.log <- glm(toxo.status ~ hum.pop.den , 
                         data = toxo_data,
                         #data = toxo_data_gil_baj, # sensitivity
                         family = binomial) 
  ...
  ### 9.6 Adujusted models: Toxo by human disturbance, sex, age.cat.dart      
    ## a) Logistic regression toxo_status by hum.pop.den
...
###############################################################################
  ...
  ### 12.1 Export toxo data to csv     
    ...
    ## a) Generate file name for toxo_data_covariates.csv
      csv.file.name.toxo.covar <- paste0(here::here
    ...
    ## b) Generate file name for lion_hy_dist_toxo.csv
      csv.file.name.lion.dist.toxo <- paste0(here::here
                                               ('data', 
                                                 'lion_hy_dist_toxo.csv'))
      
    ## c) Generate file name for lion_hy_dist_toxo_cub_sum.csv
      csv.file.name.lion.dist.toxo.cub.sum <- paste0(here::here
                                             ('data', 
